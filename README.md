...
| auto-match | Allows for auto selecting an item if the search text matches a search results attributes exactly, but only if there is one item in the list. [example](https://ghiden.github.io/angucomplete-alt/#example11) |  No | @ | true |
| auto-match-multiple | Allows for auto selecting an item if the search text matches a search results attributes exactly, regardless of how many items are in the list. |  No | @ | true |
